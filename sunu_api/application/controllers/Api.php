...
class Api extends CI_Controller {
  ...
  public function __construct(){
    ...
    $this->users = array(
      "table" => "`clients`",
      "create_fields" => ["`num_souscripteur`"],
      "create_types" => "sss",
      "read_fields" => "`*`",
      "read_key" => "`num_souscripteur`",
      "update_fields" => ["`email`"],
      "update_key" => "`num_souscripteur`",
      "delete_key" => "`num_souscripteur`"
    );
    $this->auth = array(
      "table" => "`clients`",
      "fields" => "`num_souscripteur`, `id`",
      "num_souscripteur_field" => "`num_souscripteur`",
      "id_field" => "`id`",
  ...
  public function login(){
    ...
    if($this->input->method(true) != 'POST'){
      ...
      if(check_jwt_cookie($this->auth["service_name"], $this->auth["cookie_name"])){
        ...
        $aut = (authorize($this->auth["table"], $this->auth["fields"], $_POST["num_souscripteur"],
          $this->auth["num_souscripteur_field"], $this->auth["id_field"],
          $this->auth["service_name"], $this->auth["cookie_name"]
        ));
        if($aut!==false){
          echo json_encode(get_infos_client($this->auth["table"], $_POST["num_souscripteur"],
           $this->auth["num_souscripteur_field"]));
        }
        return;
      }
  }
  ...
  public function users($param=''){
    ...
    if($param == ''){
      ...
      if($data == false)
        echo json_encode(array(
          "code" => BAD_DATA,
          "message" => "No user with this souscription number."
        ));
      else
